...
Pour finir nous avons utilisé les listes chaînées fournies par la bibliothèque standard Ada afin de parcourir les points et segments simplement. Aussi, le tri des points de gauche à droite est ainsi réalisé grâce à la procédure de tri générique de ces listes.
...
Le fichier test\_avl.adb implémente l'exemple d'arbre d'entiers du sujet afin de vérifier les valeurs attendues et en exporte un fichier \textit{test\_avl.dot}.

En effet nous avons aussi réalisé une fonction d'export \textit{dot}, \lstinline!AVL.Export!, afin de vérifier le bon équilibrage des arbres et visualiser les problèmes de comparaison de segments.

Le fichier test\_comp quant à lui, parse certains fichiers de test afin d'en extraire certains segments problématiques et en vérifier la bonne comparaison, indiquant la bonne qualité de la comparaison des segments.
